...
if upload_flag == 'Yes, upload my own Goodreads data': # upload file
	...
	if csv_file is not None:
		...
		# show predictions
		st.markdown('<span style="font-family:verdana; font-size:20pt; font-style:bold;">Your top ' + str(int(k)) + ' books are:</span>', unsafe_allow_html=True)
		st.table(topk_books[['Title','Author']])

elif upload_flag == 'No, use pre-loaded data': # use saved file
	st.markdown('<span style="font-family:verdana; font-size:16pt; font-style:bold;">Use a pre-loaded Goodreads Library</span>', unsafe_allow_html=True)
	# choose csv file
